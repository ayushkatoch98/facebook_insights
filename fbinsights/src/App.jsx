...
function App() {
  ...
  const [runOnce2, setRunOnce2] = useState(true);
  ...
  const getAllPages = () => {
    FB.api(
      "/me/accounts/?fields=name,fan_count,about,access_token&access_token=" +
        user.accessToken,
      function (response) {
  ...
  const getInsights = (
    ...
    FB.api(
      `/${pageID}/insights/?metric=page_impressions_unique,page_engaged_users,page_fans_country&period=days_28&since=${since}&until=${until}&access_token=${pageAccessToken}&period=day`,
      function (response) {
        ...
        setInsight((prev) => {
          return {
            ...
            page_fans_country:
              response.data[2]?.values != undefined
                ? [...response.data[2].values]
                : [],
  ...
  useEffect(() => {
    ...
    console.log("uasdfsaf", isLoggedIn);
    ...
    FB.login(
      function (response) {
        if (!response.authResponse) {
          ...
          Cookies.set("name", "Not Logged In");
      ...
      { scope: "read_insights,pages_show_list,pages_read_engagement,business_management" }
    );
  };
        ...
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
          <div className="grid grid-cols-1 gap-4 mb-4">
            <div className="flex flex-col gap-2 items-center justify-center">
              <div className="text-center text-red-800">
                {" "}
                <b>Note: enable pop ups, FB Login popup opens on page load</b>
                <br />
              </div>
              <div className="text-center">
                {" "}
                <b>Things that can be done to make this webpage better</b>
                <br />- Using Charts instead of Tables to display data
                <br /> - Using icons and better color theme <br /> - Using map
                for geographic
              </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                >
                  <option defaultValue="Select an options" default disabled>
                    Select a page
                  </option>
                  {pages.pages.map((item) => {
                    return (
                      <option
                        ...
                        value={item.access_token + "@@" + item.id}
                      >
                        {item.name}
                      </option>
                    );
                ...
                <div className="p-1 w-full">
                  <label className="mr-2" htmlFor="date1">
                    <b>Since:</b>
                  </label>
                  <input
                ...
                <div className="p-1 w-full">
                  <label className="mr-2" htmlFor="date2">
                    <b>Until:</b>
                  </label>
                  <input
                    className="border p-1  border-gray-500 "
                ...
                <button
                  type="submit"
                  className="bg-blue-900 p-2 text-white rounded w-full"
                >
                  Submit
                </button>
              </form>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <BasicCard
              ...
              content={insight.total_page_likes}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <CreateTable
              ...
              content={insight.page_impressions}
            />
            <CreateTable
              title="Page Engagement"
              content={insight.page_engaged_users}
            />
          </div>

          <div className="grid grid-cols-1 gap-4 mb-4">
            <CreateTableCountry
              title="page Fans Country"
              content={insight.page_fans_country}
            />
          </div>
        </div>
      </div>
